...
const ProductManagement = () => {
  ...
  const handleRecordMovement = async () => {
    ...
    try {
      ...
      if (formData.rackId) {
        try {
          const rackUpdateResponse = await fetch(`process.env.NODE_ENV === 'production' ? 'https://robridge-express.onrender.com' : 'http://localhost:3001'/api/racks/${formData.rackId}/update-quantity`, {
            method: 'POST',
            headers: {
